...
class ClarityCsvProvider implements DataSourceProviderInterface
{
    ...
    public function testConnection(): ConnectionResult
    {
        ...
        return ConnectionResult::failure(__('No CSV data available for Microsoft Clarity.', 'fp-dms'));
    }
    ...
    public function fetchMetrics(Period $period): array
    {
        ...
        if (isset($summary['daily']) && is_array($summary['daily'])) {
            ...
            $rows[] = Normalizer::ensureKeys(array_merge(
                ['source' => 'clarity', 'date' => $period->end->format('Y-m-d')],
                self::mapMetrics($summary['metrics'])
            ));
        } else {
            foreach (Dates::rangeDays($period->start, $period->end) as $date) {
                $rows[] = Normalizer::ensureKeys(['source' => 'clarity', 'date' => $date]);
            }
        }
    ...
    public static function ingestCsvSummary(string $csv): array
    {
        ...
        $daily = [];
        $totals = ['sessions' => 0.0, 'rage_clicks' => 0.0, 'dead_clicks' => 0.0];
        ...
        foreach ($rows as $row) {
            ...
            $metrics = [
                'sessions' => self::toNumber($row['sessions'] ?? ''),
                'rage_clicks' => self::toNumber($row['rage_clicks'] ?? ''),
                'dead_clicks' => self::toNumber($row['dead_clicks'] ?? ''),
            ];
