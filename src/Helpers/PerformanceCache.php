...
class PerformanceCache {
	...
	public const CACHE_GROUP_AGGREGATED = 'fp_dms_aggregated';
        ...
        /**
         * Minimum TTL enforced during schema migrations.
         */
        private const MINIMUM_TTL = 60;

        /**
         * Cache settings schema version number.
         */
        private const CACHE_SETTINGS_SCHEMA_VERSION = 2;

        /**
         * Cache index schema version number.
         */
        private const CACHE_INDEX_SCHEMA_VERSION = 2;
	 ...
	 */
        public static function get_cache_settings(): array {
                self::ensure_cache_settings_schema();
                ...
                return self::sanitize_cache_settings( $settings );
        }
	 ...
	 */
        public static function update_cache_settings( array $settings ): bool {
                ...
                $candidate = array_merge( $current, $settings );
                $sanitized = self::sanitize_cache_settings( $candidate );

                return update_option( self::OPTION_CACHE_SETTINGS, $sanitized, false );
        }

        /**
         * Execute schema upgrades for cache settings and the cache index.
         *
         * @param string $previous_version Previously installed plugin version.
         * @param string $current_version  Currently installed plugin version.
         * @return void
         */
        public static function upgrade_cache_schema( string $previous_version = '', string $current_version = '' ): void {
                self::ensure_cache_settings_schema( true );
                self::normalize_cache_index();
                self::invalidate_all();
        }
	...
	public static function warmup_cache( array $warmup_keys = [] ): array {
        ...
        /**
         * Retrieve the default cache settings array.
         *
         * @return array<string, mixed>
         */
        private static function get_default_cache_settings(): array {
                return [
                        'enabled'           => true,
        ...
        /**
         * Ensure cache settings follow the current schema before use.
         *
         * @param bool $force Whether to force re-sanitization even when already current.
         * @return void
         */
        private static function ensure_cache_settings_schema( bool $force = false ): void {
                ...
                $schema_version = isset( $settings['schema_version'] ) ? (int) $settings['schema_version'] : 1;

                if ( ! $force && $schema_version >= self::CACHE_SETTINGS_SCHEMA_VERSION ) {
                        return;
                }

                $sanitized = self::sanitize_cache_settings( $settings );
                update_option( self::OPTION_CACHE_SETTINGS, $sanitized, false );
        }

        /**
         * Normalize cache settings ensuring booleans and TTLs use safe values.
         *
         * @param array<string, mixed> $settings Settings array to sanitize.
         * @return array<string, mixed>
         */
        private static function sanitize_cache_settings( array $settings ): array {
                $defaults   = self::get_default_cache_settings();
                $sanitized  = array_merge( $defaults, $settings );
                $bool_keys  = [ 'enabled', 'use_object_cache', 'use_transients', 'auto_invalidate', 'benchmark_enabled' ];
                $ttl_keys   = [ 'default_ttl', 'metrics_ttl', 'reports_ttl', 'aggregated_ttl' ];
                $now        = time();
                $migrated_at = isset( $settings['last_migrated_at'] ) ? max( 0, (int) $settings['last_migrated_at'] ) : 0;

                foreach ( $bool_keys as $key ) {
                        $sanitized[ $key ] = isset( $sanitized[ $key ] ) ? (bool) $sanitized[ $key ] : $defaults[ $key ];
                }

                foreach ( $ttl_keys as $key ) {
                        $value = isset( $sanitized[ $key ] ) ? (int) $sanitized[ $key ] : (int) $defaults[ $key ];
                        $sanitized[ $key ] = max( self::MINIMUM_TTL, $value );
                }

                $sanitized['schema_version']      = self::CACHE_SETTINGS_SCHEMA_VERSION;
                $sanitized['cache_index_version'] = self::CACHE_INDEX_SCHEMA_VERSION;
                $sanitized['last_migrated_at']    = $migrated_at > 0 ? $migrated_at : $now;

                return $sanitized;
        }

        /**
         * Normalize the cache index removing invalid or expired entries.
         *
         * @return void
         */
        private static function normalize_cache_index(): void {
                if ( ! function_exists( 'get_option' ) || ! function_exists( 'update_option' ) ) {
                        return;
                }
                ...
                if ( empty( $index ) ) {
                        if ( ! is_array( $index ) ) {
                                update_option( self::OPTION_CACHE_INDEX, [], false );
                        }
                ...
                foreach ( $index as $group => $entries ) {
                        if ( ! is_string( $group ) || '' === trim( $group ) || ! is_array( $entries ) ) {
                                $changed = true;
                                continue;
                        }

                        $group_key    = trim( $group );
                        $group_values = [];

                        foreach ( $entries as $key => $expiration ) {
                                $normalized_key = is_string( $key ) ? trim( $key ) : (string) $key;
                                ...
                                $expires = is_numeric( $expiration ) ? (int) $expiration : 0;
                                ...
                                $group_values[ $normalized_key ] = $expires;
                        }
                        ...
                        $normalized[ $group_key ] = $group_values;
                }

                if ( $changed ) {
                        update_option( self::OPTION_CACHE_INDEX, $normalized, false );
                }
        }
